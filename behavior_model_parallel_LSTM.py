...
def main(argv):
    ...
    lstm_actions = LSTM(512, return_sequences=False, input_shape=(INPUT_ACTIONS, ACTION_EMBEDDING_LENGTH), name='lstm_actions')(embedding_actions)
    
    # Actions times branch
    input_time = Input(shape=(INPUT_ACTIONS,2), dtype='float32', name='input_time')
    lstm_time = LSTM(512, return_sequences=False, input_shape=(INPUT_ACTIONS, 2), name='lstm_time')(input_time)
    #reshape_1 = Reshape((INPUT_ACTIONS, 2))(input_time)
    #merge embeddings (5 x 50) and times (5 x 1), to have 5 x 51    
        
    concat = merge([lstm_actions, lstm_time], mode='concat', concat_axis=-1)   
    # Everything continues in a single branch
    
    dense_1 = Dense(1024, activation = 'relu',name = 'dense_1')(concat)
