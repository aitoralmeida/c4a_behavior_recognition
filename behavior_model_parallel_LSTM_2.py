...
from keras.layers import Activation, Dense, Dropout, Embedding, Input, LSTM, merge, Reshape, Flatten
...
def main(argv):
    ...
    lstm_actions = LSTM(512, return_sequences=True, input_shape=(INPUT_ACTIONS, ACTION_EMBEDDING_LENGTH), name='lstm_actions')(embedding_actions)
    
    # Actions times branch
    input_time = Input(shape=(INPUT_ACTIONS,2), dtype='float32', name='input_time')

    concat = merge([lstm_actions, input_time], mode='concat', concat_axis=-1)     
    
    # Everything continues in a single branch
    lstm_both = LSTM(512, return_sequences=False, input_shape=(INPUT_ACTIONS, 514), name='lstm_both')(concat)
    #reshape_1 = Reshape((INPUT_ACTIONS, 2))(input_time)
    #merge embeddings (5 x 50) and times (5 x 1), to have 5 x 51        
    
    dense_1 = Dense(1024, activation = 'relu',name = 'dense_1')(lstm_both)
